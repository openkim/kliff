...
class Configuration:
    ...
    def read_extxyz(self, fname):
        ...
        with open(fname, 'r') as fin:
            ...
            line = lines[1]
            self.cell = self._parse_key_value(
                line, 'Lattice', 'float', 9, fname)
            self.cell = np.reshape(self.cell, (3, 3))
            self.PBC = self._parse_key_value(line, 'PBC', 'int', 3, fname)
            self.energy = self._parse_key_value(
                line, 'Energy', 'float', 1, fname)[0]
            ...
            if self.do_order:
                if has_forces:
                    ...
                    species, coords = zip(
                        *sorted(zip(species, coords), key=lambda pair: pair[0]))
