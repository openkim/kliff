...
class NeuralNetwork(ModelTorch):
    ...
    def write_kim_model(self, path=None):
        ...
        # CMakeLists
        self.write_CMakeLists(path, modelname=modelname, version='2.0.2')

        # write parameter file
        fname = 'NN.params'
        with open(os.path.join(path, fname), 'w') as fout:
            fout.write('#' + '=' * 80 + '\n')
            fout.write('# KIM NN potential parameters, generated by KLIFF.\n')
            fout.write('#' + '=' * 80 + '\n\n')
            ...
            # header
            fout.write('#' + '=' * 80 + '\n')
            fout.write('# NN structure and parameters\n')
            fout.write('#\n')
            fout.write(
                '# Note that the NN assumes each row of the input "X" '
