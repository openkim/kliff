...
class PropertyTransform:
    ...
    @staticmethod
    def get_configuration_list(dataset: Union[List[Configuration],Dataset]) -> List[Configuration]:
        configuration_list = []
...
class NormalizedPropertyTransform(PropertyTransform):
    ...
    def transform(self, dataset: Union[List[Configuration],Dataset]):
        ...
        original_property_values = list(map(
            lambda config: getattr(config, self.property_key), configuration_list))
...
class RMSMagnitudeNormalizePropertyTransform(PropertyTransform):
    ...
    def transform(self, dataset: Union[List[Configuration],Dataset]):
        ...
        if self.keep_original:
            self.original_property_value_map = original_property_values
        self.rms_mean_magnitude = np.sqrt(np.mean(np.sum(np.square(original_property_values), 1)))
        for configuration in configuration_list:
            property_to_transform = getattr(configuration, self.property_key)
