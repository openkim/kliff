...
public:
     ...
     * \param rcut_2D
     */
    void set_cutoff(char const *name, std::size_t const Nspecies, double const *rcut_2D);
     ...
     * \param stds
     */
    void set_feature_mean_and_std(bool const normalize, int const size, double const *means, double const *stds);
     ...
     * \param std
     */
    inline void get_feature_mean_and_std(int const i, double &mean, double &std);
    ...
    /*!
     * \brief Compute the descriptor values and their derivatives w.r.t. the coordinates of
     ...
     * \param grad Flag which indicates for computing and returning both \c desc, and \c grad_desc
     *
     * \note
...
private:
     ...
     */
    void sym_g1(double const r,
                double const rcut,
                double &phi);

    void sym_d_g1(double const r,
                  double const rcut,
                  double &phi,
                  double &dphi);
     ...
     *
     * \note
     * Care must be taken when using the \c g3 function. Neighboring atoms can cancel
     * each other’s due to the existence of positive and negative function values.
     * It is recommended to use \c g3 in combination with other symmetry functions.
     */
    void sym_g3(double const kappa,
                double const r,
                double const rcut,
                double &phi);
     ...
     *
     * \param zeta Provides the angular resolution. High values yield a narrower range of nonzero
     *             symmetry function values
     * \param lambda Shifting the maxima of the cosine function (can have the values +1 or -1)
     * \param eta Controlling the radial resolution of the radial function.
     * \param r The distance between atoms \c i and \c j
     * \param rcut The cutoff radius
     * \param phi Function \c g4 value
     */
     ...
     *
     * The angular part must be symmetric with respect to \c 180 angle.
     * In function \c g5 there is no constraint on the distance between atoms resulting
     * in a larger number of terms in the summation.
     *
     * \param zeta Provides the angular resolution. High values yield a narrower range of nonzero
     *             symmetry function values
     * \param lambda Shifting the maxima of the cosine function (can have the values +1 or -1)
