...
Descriptor::Descriptor() : has_three_body_(false),
                           normalize_(true)
{
...
void Descriptor::set_cutoff(char const *name, std::size_t const Nspecies, double const *rcut_2D)
{
...
void Descriptor::add_descriptor(char const *name, double const *values, int const row, int const col)
{
...
int Descriptor::read_parameter_file(FILE *const filePointer)
{
    ...
    for (int i = 0; i < numDescTypes; ++i)
    {
        ...
        lowerCase(name);
        if (std::strcmp(name, "g1") == 0)
        {
            add_descriptor(name, nullptr, 1, 0);
        }
        else
        {
            // re-read name, and read number of param sets and number of params
            if (std::sscanf(nextLine, "%s %d %d", name, &numParamSets, &numParams) != 3)
            {
                LOG_ERROR("Unable to read the descriptor name * number of sets & parameters from line:\n" + std::string(nextLine));
                return true;
            }
            ...
            if (std::strcmp(name, "g2") == 0)
            {
                if (numParams != 2)
                {
                    LOG_ERROR("The number of params for descriptor G2 is incorrect, expecting 2, but given " + std::to_string(numParams));
                    return true;
                }
            }
            else if (std::strcmp(name, "g3") == 0)
            {
                if (numParams != 1)
                {
                    LOG_ERROR("The number of params for descriptor G3 is incorrect, expecting 1, but given " + std::to_string(numParams));
                    return true;
                }
            }
            else if (std::strcmp(name, "g4") == 0)
            {
                if (numParams != 3)
                {
                    LOG_ERROR("The number of params for descriptor G4 is incorrect, expecting 3, but given " + std::to_string(numParams));
                    return true;
                }
            }
            else if (std::strcmp(name, "g5") == 0)
            {
                if (numParams != 3)
                {
                    LOG_ERROR("The number of params for descriptor G5 is incorrect, expecting 3, but given " + std::to_string(numParams));
                    return true;
                }
            }
            else
            {
                LOG_ERROR("Unsupported descriptor `" + std::string(name) + "' from line:\n" + nextLine);
                return true;
            }
    ...
    if (normalize)
    {
        ...
        {
            LOG_ERROR("Unable to read the size of centering & normalizing data from the line:\n" + std::string(nextLine));
            return true;
        }
        else
        {
            ...
            if (size != numDescs)
            {
                LOG_ERROR("Size of centering & normalizing data inconsistent with the number of descriptors. \n Size = " +
                          std::to_string(size) + " & num_descriptors= " + std::to_string(numDescs));
                return true;
            }
        }
...
void Descriptor::set_feature_mean_and_std(bool const normalize, int const size, double const *means, double const *stds)
{
...
inline void Descriptor::get_feature_mean_and_std(int const i, double &mean, double &std)
{
    ...
    for (int jj = 0; jj < numnei; ++jj)
    {
        ...
        for (std::size_t p = 0; p < name_.size(); ++p)
        {
            ...
            for (int q = 0; q < num_param_sets_[p]; ++q)
            {
                ...
                if (name_[p] == "g1")
                {
                    grad ? sym_d_g1(rijmag, rcutij, gc, dgcdr_two) : sym_g1(rijmag, rcutij, gc);
                }
                else if (name_[p] == "g2")
                {
                    ...
                    grad ? sym_d_g2(eta, Rs, rijmag, rcutij, gc, dgcdr_two) : sym_g2(eta, Rs, rijmag, rcutij, gc);
                }
                else if (name_[p] == "g3")
                {
                    double kappa = params_[p](q, 0);

                    grad ? sym_d_g3(kappa, rijmag, rcutij, gc, dgcdr_two) : sym_g3(kappa, rijmag, rcutij, gc);
                }
            ...
            for (size_t p = 0; p < name_.size(); ++p)
            {
                ...
                for (int q = 0; q < num_param_sets_[p]; ++q)
                {
                    ...
                    if (name_[p] == "g4")
                    {
                        ...
                        grad ? sym_d_g4(zeta, lambda, eta, rvec, rcutvec, gc, dgcdr_three) : sym_g4(zeta, lambda, eta, rvec, rcutvec, gc);
                    }
                    else if (name_[p] == "g5")
                    {
                        double zeta = params_[p](q, 0);
                        ...
                        grad ? sym_d_g5(zeta, lambda, eta, rvec, rcutvec, gc, dgcdr_three) : sym_g5(zeta, lambda, eta, rvec, rcutvec, gc);
                    }
...
void Descriptor::sym_d_g1(double const r, double const rcut, double &phi, double &dphi)
{
    phi = cutoff_func_(r, rcut);
    dphi = d_cutoff_func_(r, rcut);
}

void Descriptor::sym_g2(double const eta, double const Rs, double const r, double const rcut, double &phi)
{
    phi = std::exp(-eta * (r - Rs) * (r - Rs)) * cutoff_func_(r, rcut);
}

void Descriptor::sym_d_g2(double const eta, double const Rs, double const r, double const rcut, double &phi, double &dphi)
{
    if (r > rcut)
    {
        phi = 0.0;
...
void Descriptor::sym_g3(double const kappa, double const r, double const rcut, double &phi)
{
    phi = std::cos(kappa * r) * cutoff_func_(r, rcut);
}

void Descriptor::sym_d_g3(double const kappa, double const r, double const rcut, double &phi, double &dphi)
{
                        ...
                        double &phi)
{
    ...
    else
    {
        ...
        phi = std::pow(2, 1 - zeta) *
              costerm * eterm *
              cutoff_func_(rij, rcutij) *
              cutoff_func_(rik, rcutik) *
              cutoff_func_(rjk, rcutjk);
    }
}
                          ...
                          double *const dphi)
{
    ...
    else
    {
        ...
        // dphi_dij
        dphi[0] = p2 * (dcosterm_dij * eterm * fcprod + costerm * determ_dij * fcprod + costerm * eterm * dfcprod_dij);
        // dphi_dik
        dphi[1] = p2 * (dcosterm_dik * eterm * fcprod + costerm * determ_dik * fcprod + costerm * eterm * dfcprod_dik);
        // dphi_djk
        dphi[2] = p2 * (dcosterm_djk * eterm * fcprod + costerm * determ_djk * fcprod + costerm * eterm * dfcprod_djk);
    }
}
                        ...
                        double &phi)
{
    ...
    else
    {
        ...
        phi = std::pow(2, 1 - zeta) * costerm * eterm * cutoff_func_(rij, rcutij) * cutoff_func_(rik, rcutik);
    }
}
