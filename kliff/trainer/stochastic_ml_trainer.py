...
class TorchTrainer(Trainer):
    ...
    def get_dataset(self):  # Specific to trainer
        ...
        if isinstance(dataset, Dataset):
            ...
            dataloader_train = pyg.data.DataLoader(
                dataset_train,
                batch_size=self.configuration["batch_size"],
                shuffle=True,
                num_workers=self.configuration["cpu_workers"],
            )
            dataloader_val = pyg.data.DataLoader(
                dataset_val,
                batch_size=self.configuration["batch_size"],
                shuffle=False,
                num_workers=self.configuration["cpu_workers"],
    ...
    def stop_now(self):
        ...
        ) > self.configuration["early_stopping_minimum_delta"]["validation"]:
        ...
        ) > self.configuration["early_stopping_minimum_delta"]["train"]:
        ...
        if (
            ...
            > self.configuration["early_stopping"]["early_stopping_patience"]
