...
class Descriptor(object):
    ...
    def generate_generalized_coords(self, conf, fit_forces=False, structure='bulk'):
        ...
        if fit_forces:
            gen_coords, d_gen_coords = self._cdesc.get_gen_coords_and_deri(coords.astype(np.double),
                                                                           species_code.astype(
                                                                               np.intc), neighlist.astype(np.intc),
                                                                           numneigh.astype(
                                                                               np.intc), image.astype(np.intc),
    ...
    def _set_cutoff(self):
        ...
        if self._rcut_samelayer is not None:
            rcutsym_samelayer = np.zeros(
                [num_species, num_species], dtype=np.double)
        ...
        if self._rcut_samelayer is not None:
            self._cdesc.set_cutoff_bulk_and_samelayer(
                self._cutname, rcutsym, rcutsym_samelayer)
        else:
