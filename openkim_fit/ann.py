...
def convert_raw_to_tfrecords_testset(configs, descriptor, directory='/tmp/data',
    do_generate=True, do_normalize=True, trainset_directory = None,
    do_shuffle=False, dtype=tf.float32):
  """Preprocess the data to generate the generalized coords and its derivatives,
  and store them, together with coords and label as tfRecord binary.

  This is for testset, so if do_normalize = True, one needs to provide the mean
  and standard information file of the training set, which is generated by
  calling `convert_raw_to_tfrecords'.
  ...
  do_normalize: bool
    ...
    where mean and std are read from `mean_and_std_for_kim_ann' at `trainset_directory',
    which is generated from the training set data.

  trainset_directory: string
    Directory where the mean and standard info used for normalization of the
    training set is located. Must be provided when `do_normalize' is True.
  ...
  if do_generate:
    ...
    if do_normalize:
      ...
      if trainset_directory is None:
        ...
        size = None
        data = []
        fname = os.path.join(trainset_directory, 'mean_and_std_for_kim_ann')
        shutil.copy(fname, directory)  # copy to test directory
