...
def test_configuration(e=False, f=False, s=False, order=False):
    fname = './configs_extxyz/MoS2'
    if e:
        fname += '_energy'
    if f:
        fname += '_forces'
    ...
    assert natoms == 288
    assert np.allclose(cell, [[33.337151, 0.035285, 0.03087],
    ...
    if order:
        ...
        ref_coords = [[0.051823, -0.017150, 16.736001],
    ...
    outname = fname.replace('./configs_extxyz/MoS2', 'tmp_Mos2')
    write_extxyz(outname, cell, PBC, species, coords, energy, forces, stress)


def test_dataset():
    directory = './configs_extxyz'
    tset = DataSet()
    tset.read(directory)
