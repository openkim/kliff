...
# Check that the forces and energys are the same with the KIM model when dropout is used.
# 0. modify `dropout_` function in `network.cpp` to set `debug` to true, where the drop
#    is hard-coded to be the same as the one here.
# 1. run this script
